...
function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  return (
    <div>
      ...
      <div className={`fixed inset-0 bg-slate-900 z-40 transition-opacity duration-200 lg:hidden lg:z-auto ${ sidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} aria-hidden="true"></div>

      {/* SIDEBAR */}
      <div className={`p-4 flex flex-col bg-slate-800 w-64 h-screen absolute left-0 top-0 shrink overflow-y-scroll no-scrollbar transition-all duration-200 ease-in-out z-40 lg:w-20 lg:sidebar-expanded:!w-64 lg:static lg:left-auto lg:top-auto lg:translate-x-0 lg:overflow-y-auto 2xl:!w-64 ${ sidebarOpen ? 'translate-x-0' : '-translate-x-64'}`}>
        {/* SIDEBAR HEADER */}
        <div className="flex justify-between mb-10 pr-3 sm:px-2">
          {/* CLOSE BUTTON */}
          <button className="text-slate-500 hover:text-slate-400 lg:hidden">
                      ...
                      <div className="lg:hidden lg:sidebar-expanded:block 2xl:block">
                        <ul className={`pl-9 mt-1 ${!open && 'hidden'}`}>
                          <li className="mb-1 last:mb-0">
                            <NavLink end to="/" className={({ isActive }) => 'block text-slate-400 hover:text-slate-200 transition duration-150 truncate ' + ( isActive ? '!text-indigo-500' : '')}>
